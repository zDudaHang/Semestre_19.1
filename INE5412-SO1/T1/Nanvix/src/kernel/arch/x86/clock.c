...
PRIVATE void do_clock()
{
	...
	unsigned control = prio_to_control_increment(curr_proc->priority);

	if(++curr_proc->control_counter == control) {
		curr_proc->control_counter = 0;
		curr_proc->vruntime++;
	}
		
	unsigned min_vruntime = find_min_vruntime();

	unsigned ideal_time = sched_time_slice(curr_proc);

	/* Give up processor time. */
	if ((++curr_proc->time_slice_counter >= ideal_time) && (curr_proc->vruntime > min_vruntime))
		yield();
